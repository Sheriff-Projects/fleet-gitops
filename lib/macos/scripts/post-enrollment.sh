...
launchctl asuser "${USER_ID}" sudo -u "${CURRENT_USER}" /usr/local/bin/dark-mode true
...
launchctl asuser "${USER_ID}" sudo -u "${CURRENT_USER}" /usr/local/bin/wallpaper-folder add "${WALLPAPER_FOLDER}" true
launchctl asuser "${USER_ID}" sudo -u "${CURRENT_USER}" /usr/bin/killall cfprefsd true
launchctl asuser "${USER_ID}" sudo -u "${CURRENT_USER}" /usr/bin/killall WallpaperAgent true
...
launchctl asuser "${USER_ID}" sudo -u "${CURRENT_USER}" /usr/local/bin/desktoppr "${WALLPAPER_FILE}" true
